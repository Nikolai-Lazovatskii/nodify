...
export default function RegisterScreen() {
  ...
  return (
    <View style={[s.root, isDark && s.rootDark]}>
      <Text style={[s.h1, isDark && s.h1Dark]}>{t("auth.createAccount")}</Text>
      ...
      <Pressable style={({ pressed }) => [s.btn, pressed && s.pressed]} onPress={onSubmit} disabled={busy}>
        <Text style={s.btnText}>{busy ? "..." : t("auth.signUp")}</Text>
      </Pressable>

      <Text style={[s.small, isDark && s.smallDark]}>
        {t("auth.haveAccount")} <Link href="/(auth)/login">{t("common.login")}</Link>
      </Text>
    </View>
  );
}
